...
function categorizeConversation(conv) {
  ...
  // ── Helpers ──────────────────────────────────────────────────────────────
  function userSaid(keywords) {
    return msgs.some(m => {
      if (!m.user) return false;
      const t = m.user.toLowerCase();
      return keywords.some(kw => t.includes(kw));
    });
  }
  function botSaid(keywords) {
    return msgs.some(m => {
      if (!m.bot) return false;
      const t = m.bot.toLowerCase();
      return keywords.some(kw => t.includes(kw));
    });
  }

  // ── RECLAMOS ─────────────────────────────────────────────────────────────
  // Basado en el flujo de reclamos del prompt: demora, calidad, orden incorrecta, cancelación
  const claimUserKeywords = [
    // demora / no llega
    'no llega', 'no llegó', 'lleva mucho', 'tarde', 'demoró', 'demora',
    'donde está', 'dónde está', 'cuanto falta', 'cuánto falta',
    // calidad
    'fría', 'frio', 'frío', 'frías', 'mal estado', 'podrida', 'podrido',
    'cruda', 'crudo', 'quemada', 'quemado',
    // orden incorrecta / falta algo
    'falta', 'faltó', 'incorrecto', 'incorrecta', 'equivocado', 'equivocada',
    'pedido mal', 'no pedí', 'no pedi', 'trajeron mal', 'está mal',
    'no es lo que', 'diferente a lo',
    // cancelar
    'cancelar', 'cancela', 'cancelá', 'quiero cancelar', 'no quiero más',
    // genérico de reclamo
    'reclamo', 'queja', 'problema con', 'mal servicio', 'desastre',
    'nunca más', 'pésimo', 'pesimo', 'horrible', 'muy mal',
  ];
  const hasClaim = userSaid(claimUserKeywords) ||
    botSaid(['lamento mucho', 'vamos a solucionarlo', 'te vamos a contactar']);

  // ── CONFIRMADOS ───────────────────────────────────────────────────────────
  // Detecta el PASO 4 del flujo: el bot mostró el resumen y el cliente confirmó,
  // O el bot ya cerró con las frases de cierre del prompt.
  const confirmBotKeywords = [
    // resumen de pedido enviado por el bot
    'tu pedido', 'resumen del pedido', 'resumen de tu pedido',
    'total:', 'total a pagar', 'forma de pago',
    'tiempo estimado', '50 minutos', '20 minutos',
    // frases de cierre del prompt
    'amore per i milanesi', 'gracias por elegir il milano',
    'te esperamos en av', 'cualquier consulta, no dudes',
  ];
  const confirmUserKeywords = [
    'sí, confirmo', 'si, confirmo', 'confirmo', 'confirmado',
    'dale', 'listo', 'perfecto', 'de acuerdo', 'va', 'buenísimo', 'buenisimo',
    'ok', 'okey', 'sí quiero', 'si quiero', 'está bien', 'esta bien',
  ];
  // Un pedido se considera confirmado si:
  // el bot presentó un resumen/cierre Y el cliente respondió afirmativamente,
  // O el bot envió las frases de cierre (pedido ya registrado)
  const botSentSummary = botSaid(confirmBotKeywords);
  const userConfirmed = userSaid(confirmUserKeywords);
  const hasConfirmedOrder = (botSentSummary && userConfirmed) ||
    botSaid(['gracias por elegir il milano', 'amore per i milanesi']);

  // ── NECESITAN ATENCIÓN ────────────────────────────────────────────────────
  // Situaciones donde un humano debería revisar:
  // 1. Bot pausado y hay mensajes del cliente sin respuesta manual
  // 2. Reclamo activo sin admin asignado
  // 3. El cliente preguntó algo y la última respuesta del bot fue hace rato
  //    (el bot no respondió en el último turno — msg con user pero sin bot y paused=true)
  // 4. Conversación con muchos mensajes sin procesar y sin asignar
  const isPaused = pausedSet.has(conv.jid);
  const lastMsg = msgs[msgs.length - 1];
  const lastMsgIsUnattendedUser = lastMsg && lastMsg.user && !lastMsg.bot && !lastMsg.manual;
  
  const needsAttention = !conv.processedAt && (
    // Reclamo sin admin
    (hasClaim && !conv.assignedAdmin) ||
    // Pausado y hay mensajes del cliente esperando
    (isPaused && lastMsgIsUnattendedUser) ||
    // El bot dejó un mensaje de usuario sin responder (bot pausado en ese turno)
    (lastMsgIsUnattendedUser && lastMsg.paused) ||
    // Muchos intercambios sin confirmar ni procesar ni asignar (conversación larga sin resolución)
    (msgs.length >= 6 && !hasConfirmedOrder && !conv.assignedAdmin)
  );

  return { hasConfirmedOrder, hasClaim, needsAttention };
}
...
function renderUserList() {
  ...
  for (const [jid, conv] of filteredConversations) {
    ...
    // Category badges (only show when not already filtered to that category)
    const category = categorizeConversation(conv);
    const confirmedBadge = category.hasConfirmedOrder && !conv.processedAt
      ? `<span class="badge-cat-sm badge-cat-confirmed"><i class="bi bi-check2-circle"></i> Confirmado</span>` : '';
    const claimBadge = category.hasClaim
      ? `<span class="badge-cat-sm badge-cat-claim"><i class="bi bi-exclamation-triangle-fill"></i> Reclamo</span>` : '';
    const attentionBadge = category.needsAttention && !category.hasClaim
      ? `<span class="badge-cat-sm badge-cat-attention"><i class="bi bi-bell-fill"></i> Atención</span>` : '';
    ...
    div.innerHTML = `
      ...
      ${(confirmedBadge || claimBadge || attentionBadge)
        ? `<div class="user-badges">${claimBadge}${attentionBadge}${confirmedBadge}</div>` : ''}
    `;
    div.onclick = () => {
